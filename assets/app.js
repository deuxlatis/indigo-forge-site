window.__initForge = function () {
  ...
  if (!window.Plotly) {
    gd.innerHTML =
      '<p class="fail">The 3D plotting library did not load, so the surface can\'t be drawn. ' +
  ...
  function usd(v) {
    ...
    if (a >= 1e6) {
      n = a / 1e6;
      return s + n.toFixed(n % 1 ? (n >= 10 ? 1 : 2) : 0) + "M";
    }
    if (a >= 1e3) {
      n = a / 1e3;
      return s + (n >= 100 ? n.toFixed(0) : n.toFixed(n % 1 ? 1 : 0)) + "k";
    }
  ...
  var CAM0 = {
    eye: { x: 1.59, y: 1.4, z: 0.98 },
    center: { x: 0, y: 0, z: -0.14 },
    up: { x: 0, y: 0, z: 1 },
  };
  ...
  function apply() {
    rows = RUN.passes.filter(function (r) {
      ...
      if (S.eqMin > LIM.eqMin + 1e-9 && r.eq < Math.pow(10, S.eqMin))
        return false;
      return true;
    });
  }
  ...
  function kmeans(rs, k) {
    ...
    while (C.length < k) {
      ...
      var tot = dd.reduce(function (a, b) {
          return a + b;
        }, 0),
        t = rnd() * tot,
        acc = 0,
    ...
    for (var it = 0; it < 80; it++) {
      ...
      for (j = 0; j < C.length; j++)
        if (sum[j][4]) for (i = 0; i < 4; i++) C[j][i] = sum[j][i] / sum[j][4];
      if (!moved && it > 2) break;
    }
    var groups = C.map(function (_, idx) {
      return { idx: idx, members: [] };
    });
    ...
    groups.sort(function (a, b) {
      return (
        median(
          a.members.map(function (r) {
            return r.eq;
          }),
        ) -
        median(
          b.members.map(function (r) {
            return r.eq;
          }),
        )
      );
  ...
  function ptTrace(rs, big, crange) {
    var pos = rs.filter(function (r) {
      return r.eq > 0;
    });
    var neg = rs.filter(function (r) {
      return r.eq <= 0;
    });
    var out = [];
    out.push({
      ...
      mode: "markers",
      name: "pass",
      x: pos.map(function (r) {
        return V(r, S.ax.x);
      }),
      y: pos.map(function (r) {
        return V(r, S.ax.y);
      }),
      z: pos.map(function (r) {
        return V(r, S.ax.z);
      }),
      customdata: pos.map(function (r) {
        return r.id;
      }),
      marker: {
        size: big ? 6.5 : 3.2,
        opacity: big ? 0.95 : 0.42,
        color: pos.map(function (r) {
          return V(r, S.ax.c);
        }),
        colorscale: RAMP,
        cmin: crange[0],
    ...
    if (neg.length) {
      out.push({
        ...
        mode: "markers",
        name: "stop-out",
        x: neg.map(function (r) {
          return V(r, S.ax.x);
        }),
        y: neg.map(function (r) {
          return V(r, S.ax.y);
        }),
        z: neg.map(function (r) {
          return V(r, S.ax.z);
        }),
        customdata: neg.map(function (r) {
          return r.id;
        }),
        marker: {
          size: big ? 5 : 2.6,
    ...
    clusters.forEach(function (c) {
      if (showHull > 0.01 && c.members.length >= 5) {
        out.push({
          ...
          x: c.members.map(function (r) {
            return V(r, S.ax.x);
          }),
          y: c.members.map(function (r) {
            return V(r, S.ax.y);
          }),
          z: c.members.map(function (r) {
            return V(r, S.ax.z);
          }),
        });
      }
    });
    var vis = clusters.filter(function () {
      return showMarks;
    });
    if (vis.length) {
      out.push({
        type: "scatter3d",
        mode: "markers+text",
        x: vis.map(function (c) {
          return c.cx;
        }),
        y: vis.map(function (c) {
          return c.cy;
        }),
        z: vis.map(function (c) {
          return c.cz;
        }),
        text: vis.map(function (c) {
          return c.name;
        }),
        textposition: "top center",
        textfont: { family: COND, size: 12, color: INK },
        marker: {
          size: vis.map(function (c) {
            return 9 + 22 * Math.sqrt(c.members.length / rows.length);
          }),
          color: vis.map(function (c) {
            return c.color;
          }),
          opacity: 0.85,
          line: { width: 1.5, color: WALL },
        },
        hoverinfo: "text",
  ...
  function axis(key, lo, hi) {
    ...
    if (t) {
      a.tickvals = t.tickvals;
      a.ticktext = t.ticktext;
  ...
  function paintKey(lo, hi) {
    ...
    var vals, txts, i;
    if (t) {
      vals = t.tickvals.slice();
      txts = t.ticktext.slice();
    } else {
      vals = [];
      txts = [];
      for (i = 0; i <= 4; i++) {
  ...
  function draw() {
    ...
    var sx = span(rows, S.ax.x),
      sy = span(rows, S.ax.y),
      sz = span(rows, S.ax.z),
      sc = span(rows, S.ax.c);
    paintKey(sc[0], sc[1]);
    var data = hullTraces(hull, !big)
      .concat(ptTrace(rows, big, sc))
      .concat(focusTrace());
    var layout = {
      ...
      hoverlabel: {
        bgcolor: "#1a1e22",
        bordercolor: GRID,
        font: { family: MONO, size: 11.5, color: INK },
        align: "left",
      },
      scene: {
        camera: cam,
        aspectmode: "manual",
        aspectratio: boxShape(),
        dragmode: "orbit",
  ...
  function lodFromCamera() {
    ...
    var e = cam.eye,
      n = Math.sqrt(e.x * e.x + e.y * e.y + e.z * e.z) / NORM0;
  ...
  function showPass(id) {
    var r = RUN.passes.filter(function (q) {
      return q.id === id;
    })[0];
    ...
    box.innerHTML =
      ...
      r.id +
      "</b><span>" +
      (c ? c.name : "unclustered") +
      "</span></div>" +
      '<dl class="kv">' +
      "<dt>Final equity</dt><dd>" +
      usdFull(r.eq) +
      "</dd>" +
      "<dt>Net profit</dt><dd>" +
      usdFull(r.npf) +
      "</dd>" +
      "<dt>Profit factor</dt><dd>" +
      r.pf.toFixed(2) +
      "</dd>" +
      "<dt>Max eq. drawdown</dt><dd>" +
      pct(r.ddp) +
      "</dd>" +
      "<dt>&nbsp;&nbsp;in dollars</dt><dd>" +
      usdFull(r.ddu) +
      ...
      "</dl>" +
      (r.so
        ? '<span class="badge">◆ Stopped out — account blew up</span>'
        : "") +
      (r.dup
        ? '<span class="badge">◆ Repeat of pass ' + r.dupof + "</span>"
        : "");
  }
  function clearPass() {
  ...
  function paintClusters() {
    ...
    clusters.forEach(function (c) {
      ...
      b.addEventListener("click", function () {
        var best = c.members.slice().sort(function (x, y) {
          return y.eq - x.eq;
        })[0];
        S.pinned = best.id;
        showPass(best.id);
  ...
  function paintTiles() {
    ...
    document.getElementById("t-n").innerHTML =
      n + " <small>/ " + RUN.meta.total + "</small>";
    document.getElementById("t-eq").textContent = n
      ? usd(
          Math.max.apply(
            null,
            rows.map(function (r) {
      ...
      : "—";
    document.getElementById("t-dd").textContent = n
      ? pct(
          median(
            rows.map(function (r) {
              return r.ddp;
      ...
      : "—";
    document.getElementById("t-pf").textContent = n
      ? Math.max
          .apply(
            null,
            rows.map(function (r) {
  ...
  function bindRange(id, out, fmt, set) {
    var el = document.getElementById(id),
      lab = out && document.getElementById(out);
    function upd() {
  ...
  bindRange(
    ...
    "v-pf",
    function (v) {
      return v <= LIM.pfMin + 1e-9 ? "any" : v.toFixed(2);
    },
    function (v) {
      S.pfMin = v;
      refresh();
  ...
  bindRange(
    ...
    "v-dd",
    function (v) {
      return v >= LIM.ddMax - 1e-9 ? "any" : v.toFixed(0) + "%";
    },
    function (v) {
      S.ddMax = v;
      refresh();
  ...
  bindRange(
    ...
    "v-eq",
    function (v) {
      return v <= LIM.eqMin + 1e-9 ? "any" : usd(Math.pow(10, v));
    },
    function (v) {
      S.eqMin = v;
      refresh();
  ...
  document.getElementById("hulls").addEventListener("change", function (e) {
    S.hulls = e.target.checked;
    draw();
  });

  document.getElementById("ex-so").addEventListener("change", function (e) {
    S.exSo = e.target.checked;
    refresh();
  });
  document.getElementById("ex-dup").addEventListener("change", function (e) {
    S.exDup = e.target.checked;
    refresh();
  });
  ...
  Object.keys(modes).forEach(function (id) {
    document.getElementById(id).addEventListener("click", function () {
      ...
      Object.keys(modes).forEach(function (o) {
        document
          .getElementById(o)
          .setAttribute("aria-pressed", String(o === id));
      });
  ...
  gd.addEventListener("mouseleave", function () {
    if (S.pinned != null) showPass(S.pinned);
    else clearPass();
  });
  ...
  function attach() {
    ...
    gd.on("plotly_click", function (e) {
      ...
      if (p && p.customdata != null) {
        ...
        if (S.pinned == null) clearPass();
        else showPass(S.pinned);
        draw();
      }
    });
  ...
  function describe(meta) {
    ...
    if (meta.total)
      bits.push(
        nice(meta.total) +
          "-pass " +
          (meta.method || "").toLowerCase() +
          " optimization",
      );
    if (meta.start && meta.end)
      bits.push("<b>" + meta.start + " → " + meta.end + "</b>");
  ...
  function fitControls() {
    var ps = RUN.passes;
    if (!ps.length) return;
    var pf = ps.map(function (r) {
      return r.pf;
    });
    var dd = ps.map(function (r) {
      return r.ddp;
    });
    var eq = ps.map(function (r) {
      return Math.max(r.eq, 1);
    });
    ...
    if (+kel.value > kmax) {
      kel.value = kmax;
      kel.dispatchEvent(new Event("input"));
    }
  }
  ...
  function paintMeta() {
    ...
    document.getElementById("so-note").textContent =
      ...
      " pass" +
      (m.stopouts === 1 ? "" : "es") +
      " blew the account (equity drawdown ≥ 100%)";
    document.getElementById("dup-note").textContent =
      m.dups +
      " pass" +
      (m.dups === 1 ? "" : "es") +
      " reproduce an earlier pass's exact metrics";
    ...
    document.getElementById("hy-note").innerHTML =
      (m.contiguous
        ? "Pass IDs run " +
          ...
          "–" +
          m.idmax +
          " with no gaps or repeats, so this export holds <b>one run only</b>."
        : "Pass IDs are <b>not contiguous</b> (" +
          m.idmin +
          "–" +
          m.idmax +
          " with gaps), which can mean results from an earlier optimization were left in this file.") +
  ...
  function activate(report, entry) {
    ...
    document.getElementById("rep-count").textContent =
      nice(report.passes.length) + " passes";
  ...
  function getBundled() {
    if (window.__BUNDLED) {
      return Promise.resolve(
        window.__BUNDLED.map(function (b, i) {
          return {
            key: "bundled:" + i,
            label: b.meta.name,
            inline: b,
  ...
  function selectKey(key) {
    var b = bundled.filter(function (e) {
      return e.key === key;
    })[0];
    if (b) return loadBundled(b);
    if (key.indexOf("saved:") === 0)
      return loadSaved({ key: key, id: key.slice(6), saved: true });
    return Promise.resolve();
  }
      ...
      if (last) {
        ...
        toast(
          ok +
            ...
            (ok === 1 ? "" : "s") +
            " added — " +
            nice(last.passes.length) +
            " passes",
        );
        return activate(last, {
          key: "saved:" + last.id,
          label: last.meta.name,
          saved: true,
          id: last.id,
        });
      }
  ...
  document.getElementById("rep-pick").addEventListener("change", function (e) {
    selectKey(e.target.value).catch(function (err) {
      toast("Could not load that report: " + err.message, true);
    });
  });
